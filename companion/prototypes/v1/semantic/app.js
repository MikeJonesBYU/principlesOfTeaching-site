/* Prototype v1, alternative build — Team A's app with a semantic search
   engine. Same data file, same three views, same click-and-query log; the
   one change is the search. The hand-built concept vocabulary is grown into
   a precomputed meaning index (semantic-index.js): every plausible teacher
   word and phrase was compared against every skill's wording, its manual
   Define text, and an authored situation sentence per branch — by a sentence
   -embedding model run once at build time on the instructor's machine.

   At lookup time there is no model, no server, and no AI call: search is
   dictionary lookups and addition, same as ever. The vocabulary the studies
   committed by hand keeps its hand-built targets; the model only speaks
   where the studies were silent. Build: tools/build_semantic_index.py. */
(function () {
  "use strict";
  ...
  // Team A's stoplist plus the pronouns and question words that were
  // producing junk literal hits ("their" matching half the manual). The
  // alternative build's one change to the literal layer.
  ...
  /* ============================================================== SEARCH ===
     ...
     1. Literal. Unchanged from the original build: every typed word matched
        against the block's label, its manual sentence, and the names of the
        categories, groups and moments it carries. Word-prefix matching.
     2. Semantic. The query is cut into every phrase and word the meaning
        index knows (longest phrases first, and the words inside a known
        phrase still count — they point at the same meaning). Each known term
        carries a precomputed list of skills it is close in meaning to, with
        a weight from 1 to 100 shaped at build time. A term's contribution is
        weight/100 x 6, so typing a skill's meaning exactly is worth the same
        as typing its label exactly — and a loose synonym is worth less than
        the manual's own words. Nothing is computed at lookup time beyond
        addition; every hit carries its reason on screen.
     ======================================================================= */
  ...
  // Cut the query into every known term: at each word, try the 4-, 3-, 2-,
  // then 1-word phrase starting there against the meaning index. A word is
  // "unknown" only if no known segment covers it.
  ...
  function viewHome() {
    ...
    return "<h1>Find something to try on Sunday</h1>" +
      ...
      "because teachers reach for it from more than one question. " +
      "In this build the search box matches what you mean, not just the " +
      "words you use &mdash; type it the way you&rsquo;d say it.</p>" +
  ...
  function viewHow() {
    return crumbs([{ text: "Home", href: "#/" }, { text: "About this search" }]) +
      here("Alternative build", "How this search works",
           "Meaning computed once, ahead of time. Looked up in your browser.") +
      '<div class="section"><h2>What happens when you type</h2>' +
      '<p class="meta">Your words are matched two ways. Literal: every word is ' +
      "matched against each skill\u2019s label, its manual sentence, and where " +
      "it is filed &mdash; exactly as in the original build. Semantic: every " +
      "phrase and word the meaning index knows adds the skills it is close in " +
      "meaning to, each with a precomputed weight. The two layers add; an " +
      "exact label match still outranks a loose synonym. Results show their " +
      "reason (\u201cyour \u2018restless\u2019 is close in meaning " +
      "to&hellip;\u201d).</p></div>" +
      '<div class="section"><h2>Where the meaning comes from</h2>' +
      '<p class="meta">' + S.termCount + " words and phrases were compared " +
      "against every skill\u2019s wording, its manual Define text, and a " +
      "short situation sentence written for each branch of the hierarchy " +
      "(\u201cThe class is bored and restless \u2014 phones out, eyes " +
      "wandering&hellip;\u201d). The comparing was done once, at build time, " +
      "by a sentence-embedding model (" + esc(S.model) + "; built " + esc(S.built) +
      "). Only the resulting lookup table ships. The browser never runs a " +
      "model and never calls a server &mdash; a search is dictionary lookups " +
      "and addition, and works offline.</p></div>" +
      '<div class="section"><h2>What the studies committed stays committed</h2>' +
      '<p class="meta">The vocabulary our card sort and tree test committed by ' +
      "hand (\u201cis it the spirit\u201d, \u201cbored\u201d, " +
      "\u201cobject lesson\u201d&hellip;) keeps its hand-built targets at " +
      "full strength. The model only speaks where the studies were " +
      "silent.</p></div>" +
      backnav([{ text: "\u2190 Home", href: "#/" }]);
  }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (act === "try") {
      ...
      download("semantic-v1-log-" + stamp() + ".json", JSON.stringify(readLog(), null, 2), "application/json");
    } else if (act === "log-csv") {
      download("semantic-v1-log-" + stamp() + ".csv", logCsv(readLog()), "text/csv");
